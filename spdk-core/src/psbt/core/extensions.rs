...
/// 2. Taproot BIP32 derivation (tap_key_origins for P2TR)
...
pub fn get_input_pubkey(psbt: &SilentPaymentPsbt, input_idx: usize) -> Result<PublicKey> {
    ...
    // Method 2: Extract from Taproot BIP32 derivation (tap_key_origins for P2TR)
    ...
    Err(Error::Other(format!(
        "Input {} missing public key (no SP spend, BIP32 derivation, or partial signature found)",
        input_idx
    )))
}
