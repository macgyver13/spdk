...
fn validate_ecdh_coverage(psbt: &SilentPaymentPsbt) -> Result<()> {
    ...
    for scan_key in &scan_keys {
        ...
        // Must have at least one ECDH share for this scan key
        ...
        // If using per-input shares (not global), and outputs for this scan key are computed,
        // require complete ECDH coverage of all eligible inputs
        if has_input_ecdh && !has_global_ecdh {
            ...
            if has_computed_outputs {
                // All eligible inputs must have ECDH share for this scan key
